...
**Museum hub ([src/museum/](src/museum/))** — `Museum.js` ("Aking Museo") is a self-contained dark-gallery hub scene (its own `THREE.Scene`) with empty art frames/slots tracked per zone (`slotsByZone`), pedestals, and wall portals to each zone (locked flags gate progress; only zone1 starts unlocked). It's geometry-only — no camera/cutscene logic of its own; static geometry is frozen (`matrixAutoUpdate = false`) since only lights/material colors change post-build. `PortalVortex.js` exports `createVortexMaterial(aspect)`, a shared swirling-spiral `ShaderMaterial` used for every open portal's corridor panel; Museum places the meshes and updates `uTime` per frame.
