// ============================================================
// API MANAGER — GameOn Portal authorization + campaign reward
// ============================================================
...
const PLACEHOLDER_GAME_IDS = new Set(['', 'YOUR_GAME_ID']);
const SESSION_TOKEN_LIMIT = 4096;
...
function normalizeToken(value) {
  ...
  return token && token.length <= SESSION_TOKEN_LIMIT ? token : null;
}

export class APIManager {
  constructor(config, dependencies = {}) {
    this.baseUrl = normalizeBaseUrl(config?.BASE_URL);
    this.gameId = String(config?.GAME_ID || '').trim();
    this.pollIntervalMs = Number(config?.POLL_INTERVAL_MS) > 0
    ...
    this.sessionToken = normalizeToken(this._readStorage(this.storageKeys.sessionToken));
    this.signinUrl = null;
    this.pendingUnlock = this._readStorage(this.storageKeys.pendingUnlock) === '1';
    this.state = {
      connection: this.isConfigured
        ? (this.sessionToken ? 'pending' : 'disconnected')
        : 'unconfigured',
  ...
  get isConfigured() {
    return Boolean(this.baseUrl && !PLACEHOLDER_GAME_IDS.has(this.gameId));
  }
  ...
  async connect() {
    if (!this.isConfigured || this.disposed) {
      this._setState({
        connection: this.disposed ? this.state.connection : 'unconfigured',
        error: this.disposed ? null : 'GameOn Portal is not configured.',
      });
      return false;
    }

    // Reserve a tab inside the click gesture. Navigating after the POST resolves
    // avoids popup blockers without putting the token in a URL or local storage.
    let reservedWindow = null;
    try {
      reservedWindow = this.openWindow?.('about:blank', '_blank');
      if (reservedWindow) reservedWindow.opener = null;
    } catch (error) {
      // A blocked popup is recoverable through openSignIn once signinUrl exists.
    }
    return this._createSession(reservedWindow);
  }

  openSignIn() {
    if (!this.signinUrl || !this.openWindow || this.disposed) return false;
    try {
  ...
  async _createSession(reservedWindow = null) {
    ...
    try {
      ...
      if (requestGeneration !== this.generation || this.disposed) return false;
      this._setState({ connection: 'error', error: 'Could not create a GameOn session.' });
  ...
  async _unlockArtifact() {
    try {
      ...
      if (this.disposed) return false;
      this._setState({ unlock: 'error', error: 'Campaign reward unlock will be retried.' });
  ...
  _schedulePoll(requestGeneration) {
    this._cancelPoll();
    if (this.disposed || !this.setTimer) return;
  ...
  async _readJson(response) {
    try {
      ...
      throw new Error('GameOn Portal response is not valid JSON.');
    }
  }

  _validateSessionResponse(data) {
    const sessionToken = normalizeToken(data?.sessionToken);
    if (!sessionToken || typeof data?.signinUrl !== 'string') {
    ...
    return { sessionToken, signinUrl: signinUrl.href };
  }
  ...
  _navigateReservedWindow(reservedWindow, signinUrl) {
    ...
    try {
      if (reservedWindow.closed) return;
      reservedWindow.location.replace(signinUrl);
    } catch (error) {
      try { reservedWindow.location.href = signinUrl; } catch (navigationError) { /* user can retry */ }
    }
  }
